        ...
        for (const item of data) {
            ...
            while (true) {
                ...
                if (data2.length === 1) {
                    ...
                    const existingStock = await StockHold.findOne({ symbol: stockHoldItem.symbol });
                    if (!existingStock) {
                        ...
                        if (stockHoldItem.type === 'buy') {
                            ...
                            const newPrice = 
                                (existingStock.price * existingStock.quantity + stockHoldItem.price * stockHoldItem.quantity) /
                                (existingStock.quantity + stockHoldItem.quantity);
                            const newQuantity = existingStock.quantity + stockHoldItem.quantity;
                        ...
                        } else if (stockHoldItem.type === 'sell') {
                            // Handle sell: Reduce quantity
                            const newQuantity = existingStock.quantity - stockHoldItem.quantity;

                            if (newQuantity < 0) {
                                console.error("Error: Sell quantity exceeds current stock quantity");
                                break;
                            }
